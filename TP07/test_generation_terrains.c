...
int main(int argc, char **argv) {
  ...
  // A compléter
    ...
    for (i = 0; i < N; i++) {
        ...
        dObst_obtenue = (float)nb_cases_occupees(&T) / (float)(l * h);
        somme_densite = somme_densite + dObst_obtenue;
        fprintf(resFile, "%f\n\n", dObst_obtenue);
    }
    fprintf(resFile, "%f\n\n", (float) somme_densite/(float)N);
    fprintf(resFile, "%f\n\n", (float)N*100/(float)gener);
  // A compléter
