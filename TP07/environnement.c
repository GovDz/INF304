...
erreur_terrain initialise_environnement(Environnement *envt,
  ...
  envt->obs = initial(); // Initialisation de l'état ( état initial )
...
void gauche_envt(Environnement *envt) { envt->obs = transition(envt->obs,G); tourner_a_gauche(&(envt->r)); }

/* Tourner le robot à droite */
void droite_envt(Environnement *envt) { envt->obs = transition(envt->obs,D);tourner_a_droite(&(envt->r)); }
...
int check_obs(const Environnement* envt) { // We dont change the value of envt
    return BoolFinal(envt->obs);
}